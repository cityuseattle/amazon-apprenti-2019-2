...
export class BookFormComponent implements OnInit {
  ...
  handleSave() {
    let message: string;
    // If the form input values are invalid, show a snackbar
    if (this.title === '' || this.isbn === '' || this.author === '' )
      ...
      message = 'Please offer a price greater than 0.'
    else {
      // Call the add book API and reset all form input values
      message = 'Book is added successfully.';
      this.apiService.addOrUpdateBook({
        title: this.title, isbn: this.isbn, author: this.author, picture: this.picture, price: this.price, _id: this.bookId,
      }).subscribe(() => {
      this.title = '';
      this.isbn = '';
      this.author = '';
